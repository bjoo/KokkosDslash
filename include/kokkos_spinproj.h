		       ...
		       HalfSpinorSiteView<T2>& spinor_out)
{
  ...
  for(int color=0; color < 3; ++color) {
    ...
    A_add_sign_iB<FType,isign>(spinor_out(color,0), in(color,0), in(color,3) );
  ...
  for(int color=0; color < 3; ++color) {
    ...
    A_add_sign_iB<FType,isign>(spinor_out(color,1), in(color,1), in(color,2));
  }
}
...
KOKKOS_INLINE_FUNCTION
			 ...
			 HalfSpinorSiteView<T2>& spinor_out)
  {
    ...
    for(int color=0; color < 3; ++color) {
      ...
      A_add_sign_B<FType,-isign>(spinor_out(color,0),in(color,0),in(color,3));
    }
    for(int color=0; color < 3; ++color) {
      ...
      A_add_sign_B<FType,isign>(spinor_out(color,1), in(color,1),in(color,2));
    }
  }
			 ...
			 HalfSpinorSiteView<T2>& spinor_out)
  {
    ...
    for(int color=0; color < 3; ++color) {
      ...
      A_add_sign_iB<FType,isign>(spinor_out(color,0),in(color,0),in(color,2));
    }
    ...
    for(int color=0; color < 3; ++color ) {
      ...
      A_add_sign_iB<FType,-isign>(spinor_out(color,1), in(color,1),in(color,3));
    }
  }
			 ...
			 HalfSpinorSiteView<T2>& spinor_out)
  {
    ...
    for(int color=0; color < 3; ++color) {
      ...
      A_add_sign_B<FType,isign>(spinor_out(color,0), in(color,0), in(color,2));
    }
    ...
    for(int color=0; color < 3; ++color) {
      ...
      A_add_sign_B<FType,isign>(spinor_out(color,1), in(color,1), in(color,3));
    }
  }
...
//#ifndef MG_FLAT_PARALLEL_DSLASH
	...
	Kokkos::parallel_for(num_sites, KOKKOS_LAMBDA(const int i) {
			...
			SpinorSiteView<T2> in;
			load<T2,T>(in,spinor_in,i);
		if( dir == 0) {
		  //			KokkosProjectDir<T,0>(spinor_in,plus_minus,res,i);
		  KokkosProjectDir0<T,T2,isign>(in, res);
		}
		else if (dir == 1) {
		  //			KokkosProjectDir<T,1>(spinor_in,plus_minus,res,i);
		  KokkosProjectDir1<T,T2,isign>(in, res);
		}
		else if (dir == 2 ) {
		  KokkosProjectDir2<T,T2,isign>(in, res);
		}
		else {
